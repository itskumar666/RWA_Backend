...
app.post('/assets/register', upload.array('files', 10), async (req: Request, res: Response) => {
  console.log('=== ASSET REGISTRATION ENDPOINT CALLED ===');
  try {
  const { assetType, assetName, valueUSD, owner, isLocked: isLockedRaw } = req.body as { assetType: string; assetName: string; valueUSD: string; owner?: string; isLocked?: string };
    ...
    console.log(`Received ${files.length} files for asset: ${assetName}`);
  ...
  // 2) Upload files to Pinata IPFS (temporary local storage for processing)
  console.log('Starting file uploads to Pinata IPFS...');
  ...
  for (const f of files) {
    ...
    const tempPath = path.join(assetDir, safeName);
    
    // Write temporarily for processing
    fs.writeFileSync(tempPath, f.buffer);
    tempFiles.push(tempPath);
    
    try {
      console.log(`Uploading ${safeName} to Pinata...`);
      const ipfsUrl = await uploadToPinata(f.buffer, safeName);
      console.log(`Successfully uploaded ${safeName} to IPFS: ${ipfsUrl}`);
      ipfsUrls.push(ipfsUrl);
      
      // Delete local file immediately after successful upload
      fs.unlinkSync(tempPath);
      console.log(`✅ Deleted local file: ${safeName}`);
    } catch (err) {
      console.error(`Failed to upload ${safeName} to Pinata:`, err);
      ipfsUrls.push(''); // If upload fails, leave blank
      
      // Still delete the temp file even if upload failed
      try {
        fs.unlinkSync(tempPath);
        console.log(`🗑️ Deleted temp file after upload failure: ${safeName}`);
      } catch (unlinkErr) {
        console.error(`Failed to delete temp file ${safeName}:`, unlinkErr);
      }
    }
  }

  // Clean up empty asset directory if all files were successfully uploaded and deleted
  try {
    const remainingFiles = fs.readdirSync(assetDir).filter(f => f !== 'metadata.json');
    if (remainingFiles.length === 0) {
      console.log(`All files uploaded to IPFS. Asset directory: ${assetDir} contains only metadata.json`);
    }
  } catch (err) {
    console.error('Error checking asset directory:', err);
  }

  // Save metadata with only IPFS URLs (no local files)
  const metadataFile = path.join(assetDir, 'metadata.json');
  const metadata = {
    assetId: id,
    assetName,
    assetType: Number(assetType || 0),
    valueUSD: valueUSD || '0',
    isLocked,
    owner: ownerAddr,
    ipfsUrls: ipfsUrls.filter(url => url), // Remove empty URLs
  ...
  res.json({ assetId: id, isLocked, txHash: hash, ipfsUrls });
  } catch (e: any) {
...
// List assets with IPFS URLs (enhanced version)
app.get('/assets/list-with-ipfs', async (req: Request, res: Response) => {
  try {
    const owner = (req.query.owner as string) || '';
    if (!owner || !/^0x[0-9a-fA-F]{40}$/.test(owner)) return res.status(400).json({ error: 'valid owner query param required' });
    ...
    // Normalize BigInt fields and add IPFS URLs from metadata files
    const assets = (data as any[]).map((a: any) => {
      const assetId = typeof a.assetId === 'bigint' ? a.assetId.toString() : String(a.assetId);
      
      // Try to read metadata file with IPFS URLs
    ...
    res.status(500).json({ error: e?.shortMessage || e?.message || 'list with ipfs failed' });
  }
});

// Get asset metadata including IPFS URLs
app.get('/assets/metadata/:assetId', async (req: Request, res: Response) => {
  try {
